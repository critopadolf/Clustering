...
class cluster:
	...
	def group(self,inp,groupCount,rounds):
		...
		for r in range(self.rn):
			...
			with mp.Pool(processes=4) as p:
				self.groupNums = list(p.map(self.findGroup,self.inp))
				p.close()
				p.join()
			#find new centers
	...
	def add(self,v1,v2):
		#mp.freeze_support()
		f = lambda x, y: x + y
		with mp.Pool(processes=len(v1)) as p:
			return p.starmap(f,zip(v1,v2))

	def mult(self,v,c):
		#mp.freeze_support()
		f = lambda x: x * c
		with mp.Pool(processes=len(v)) as p:
			out = p.map(f,v)
		return out
	'''
	def mult(self,v,c):
